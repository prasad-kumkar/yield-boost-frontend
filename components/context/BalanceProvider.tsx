...
function BalanceContextProvider({ children }: any) {
    ...
    React.useEffect(() => {
        if(status == Status.NOT_FETCHING && selectedLendingMarket.length > 0) {
            fetchBalances(address);
        }
    }, [(selectedLendingMarket ?? []).length, address, status])
	...
	const fetchBalances = async (_address?: string) => {
        ...
        helper.callStatic.aggregate(calls).then(async (res: any) => {
            ...
            console.log("balances", newBalances, index, calls.length);
